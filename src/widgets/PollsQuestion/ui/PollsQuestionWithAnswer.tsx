...
import {
  ...
  IPollQuestions,
  IPollsChoice,
  IPollsGetCommentById,
  IPollsResponseAnswer,
  IPollsUpdateAnswerBody,
  IPollsUpdateCommentBody,
  IStatus,
...
interface IPollsQuestionWithAnswerProps {
  ...
  status: IStatus;
  answer: IPollsResponseAnswer;
  initialComment?: IPollsGetCommentById;
}

export const PollsQuestionWithAnswer: FC<IPollsQuestionWithAnswerProps> = ({
  questions,
  setStage,
  ...
  // Свой ответ
  const [ownAnswer, setOwnAnswer] = useState(() => answer.own_answer || "");
  const [selectedOwnAnswer, setSelectedOwnAnswer] = useState(
    Boolean(answer.own_answer),
  );
  const [currentAnswer, setCurrentAnswer] = useState<IPollAnswer[] | null>(
    () => (answer.choices ? answer.choices : null),
  );

  // Комментарий
  const [comment, setComment] = useState(() => initialComment?.body || "");
  ...
  const { mutate: updateAnswer } = useMutation({
    mutationFn: async ({
      ...
      answerId: string;
      body: IPollsUpdateAnswerBody;
    }) => updateAnswerForPollQuestion({ answerId })(body),
    onSuccess: (response) => {
  ...
  const { mutate: updateComment } = useMutation({
    mutationFn: async ({
      ...
      commentId: string;
      body: IPollsUpdateCommentBody;
    }) => updateCommentForPollQuestion({ commentId })(body),
    onSuccess: (response) => {
      queryClient.invalidateQueries({
        queryKey: ["poll-comment", { questionId: response.question_id }],
      });
    },
    onError: (error) => {
      console.log(error);
  ...
  const { mutate: deleteComment } = useMutation({
    mutationFn: async ({ commentId }: { commentId: string }) =>
      deleteCommentForPollQuestion({ commentId }),
    onSuccess: (response) => {
      queryClient.invalidateQueries({
        queryKey: ["poll-comment", { questionId: response.question_id }],
      });
    ...
    onError: (error) => {
      ...
      Toast.show({
        type: "error",
        text1: "Ошибка удаления комментария",
      });
    },
  });
  ...
  const handlePressContinue = () => {
    if (comment && initialComment?.id && comment !== initialComment.body) {
      console.log("update comment");
      updateComment({
        commentId: String(initialComment.id),
        body: {
    ...
    if (!comment && initialComment?.id) {
      ...
      console.log(initialComment.id);

      deleteComment({ commentId: String(initialComment.id) });
    }

    // Через тернарники к звездам
    const choices = currentAnswer?.length
      ? currentQuestion.type === "RADIO"
        ? [{ id: currentAnswer[0].id }]
        : currentQuestion.type === "CHECKBOX"
          ? currentAnswer.map((answer) => ({ id: answer.id }))
          : [{ title: currentAnswer[0].title }]
      : [];

    console.log("selected choices", choices);
    console.log("selectedOwnVariant", {
